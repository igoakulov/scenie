...
describe("validateScene", () => {
  it("requires bindInput when host.camera is false", async () => {
  ...
  it("accepts a constructed Scene without an export", async () => {
    ...
    await writeFile(join(dir, "host.js"), `export function params() { return []; }\n`, "utf8");
  ...
  it("fails when writable cards have no applyParams", async () => {
    const workspace = await mkdtemp(join(tmpdir(), "scenie-val-"));
    const dir = await copyFixture("valid-basic", workspace, "no-apply");
    await writeFile(
      join(dir, "scene.js"),
      `import * as THREE from "three";\nexport const scene = new THREE.Scene();\n`,
      "utf8",
    );
    const result = await validateScene(workspace, "no-apply");
    assert.equal(result.ok, false);
    assert.ok(result.issues.some((i) => i.path === "scene.applyParams"));
  });

  it("warns when scene.js calls requestAnimationFrame at load", async () => {
    const workspace = await mkdtemp(join(tmpdir(), "scenie-val-"));
    const dir = await copyFixture("valid-basic", workspace, "raf");
    await writeFile(join(dir, "host.js"), `export function params() { return []; }\n`, "utf8");
    await writeFile(
      join(dir, "scene.js"),
    ...
    const result = await validateScene(workspace, "raf");
    assert.equal(result.ok, true, JSON.stringify(result.issues, null, 2));
    assert.ok(
      result.issues.some(
        (i) =>
          i.level === "warning" &&
          i.path === "scene" &&
          /requestAnimationFrame/.test(i.message),
      ),
    );
  });
});
